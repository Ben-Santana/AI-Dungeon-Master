...
const SideBarSpells = ({ player }: { player: Adventurer }) => {
    return <div className="custom_bg-gray rounded-sm p-3 m-1 text-left" >
        {player.spells.map((spell: Spell) =>
...
const Title = ({ player }: { player: Adventurer }) => {
    const styles: string = `${ player.class === 'knight' ? 'bg-zinc-600': ''}
                            ${ player.class === 'wizard' ? 'bg-violet-950': ''}
                            ${ player.class === 'artificer' ? 'bg-amber-900': ''}`;
    

    return <div className={`${styles} rounded-md p-3 m-1 text-left text-center custom_shadow-inner`} >
        <strong className="text-white text-3xl">{player.name}</strong>
    </div>
}

const SideBarSkills = ({ player }: { player: Adventurer }) => {
    return <div className="custom_bg-gray rounded-sm p-3 m-1 text-left" >
...
const SideBarInventory = ({ player }: { player: Adventurer }) => {
    return <div className="custom_bg-gray rounded-sm p-3 m-1 text-left" >
...
const SideBarStats = ({ player }: { player: Adventurer }) => {
...
const SideBarHealth = ({ player }: { player: Adventurer }) => {
...
const SideBarCoins = ({ player }: { player: Adventurer }) => {
...
export default function SideBar({ player }: { player: Adventurer }) {
    return  player? <div className="text-center overflow-auto overscroll-auto scrollbar-thumb:!rounded h-full no-scrollbar custom_enchanted-font ">
                        <Title player={player}></Title>
                        <SideBarHealth player={player}></SideBarHealth>
                        <strong className="text-white text-3xl">Spells</strong>
                        <SideBarSpells player={player}></SideBarSpells>
                        <br />
                        <strong className="text-white text-3xl">Skills</strong>
                        <SideBarSkills player={player}></SideBarSkills>
                        <br />
                        <strong className="text-white text-3xl">Inventory</strong>
                        <SideBarInventory player={player}></SideBarInventory>
                        <br />
                        <strong className="text-white text-3xl">Stats</strong>
                        <SideBarStats player={player}></SideBarStats>
                        <br />
                        <SideBarCoins player={player}></SideBarCoins>
                        <br />
